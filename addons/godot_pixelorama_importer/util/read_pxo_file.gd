...
static func read_pxo_file(source_file: String, image_save_path: String):
	...
	if err != OK:
		...
		return _read_pxo_file_v1(file, image_save_path)
	return _read_pxo_file_v2(zip_reader, image_save_path)


static func _read_pxo_file_v1(file: FileAccess, image_save_path: String):
...
static func _read_pxo_file_v2(zip_reader: ZIPReader, image_save_path: String):
	var result = Result.new()
	...
	for frame_index in range(frame_count):
		...
		for layer_index in range(layer_count):
			...
			if layer.visible and opacity > 0.0:
				...
				if cel_data.is_empty():
					printerr(
						(
							"Failed to read cel data for frame %d, layer %d"
							% [frame_index + 1, layer_index + 1]
						)
					)
					continue

				var cel_img = Image.create_from_data(
					size.x, size.y, false, Image.FORMAT_RGBA8, cel_data
				)
		...
		spritesheet.blit_rect(
			frame_img, Rect2(Vector2.ZERO, size), Vector2(size.x * frame_index, 0)
		)
